...
class Layerbridge extends Component {
  ...
  async fetchToESNBlocks() {
    if(this.state.blockNumberETH && this.state.latestBlockNumberOnESNContract){
        let res;
        try {
          res = await Apis.fetchLayer2ToESNBlock((this.state.blockNumberETH ?? 0) -
                  ...
                  <table className="es-transaction striped bordered hover table">
                                      ...
                                      <tr>
                                        ...
                                        <td className="noborder">
                                          <p className="explr-purple-text">
                                            ...
                                            {this.state.latestBlockNumberOnESNContract ?? 'Loading...'}
                    ...
                    {this.state.toESNblocks.isLoading ? 
                      ...
                      <tr>
                        ...
                        <td>{block.block_number <= this.state.latestBlockNumberOnESNContract 
                          ? 'Finalized!'
                          :
                          <>[{block?.proposalValidators?.map((validator) => validator?.address.substr(0,6)).join(',')}]</>
                        }</td>
                      </tr>
                    )
                      :
